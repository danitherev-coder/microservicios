...
public class UsuarioController {
    ...
    private ResponseEntity<List<Carro>> fallBackGetCarros(@PathVariable Long usuarioId, RuntimeException ex){
        return new ResponseEntity("El usuario: " + usuarioId + " tiene los carros en el taller", HttpStatus.OK);
    }
    private ResponseEntity<List<Moto>> fallBackGetMotos(@PathVariable Long usuarioId, RuntimeException ex){
        return new ResponseEntity("El usuario: " + usuarioId + " tiene las motos en el taller", HttpStatus.OK);
    }
    private ResponseEntity<Carro> fallBackSaveCarros(@PathVariable Long usuarioId, @RequestBody Carro carro, RuntimeException ex){
        return new ResponseEntity("El usuario: " + usuarioId + " no pudo guardar el carro", HttpStatus.OK);
    }
    private ResponseEntity<Moto> fallBackSaveMotos(@PathVariable Long usuarioId, @RequestBody Moto moto, RuntimeException ex){
        return new ResponseEntity("El usuario: " + usuarioId + " no pudo guardar la moto", HttpStatus.OK);
    }
    private ResponseEntity<Map<String, Object>> fallBackGetVehiculos(@PathVariable Long usuarioId, RuntimeException ex){
